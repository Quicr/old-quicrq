...
uint8_t* quicrq_repair_msg_encode(uint8_t* bytes, uint8_t* bytes_max, uint64_t message_type,
...
const uint8_t* quicrq_msg_decode(const uint8_t* bytes, const uint8_t* bytes_max, quicrq_message_t* msg)
{
    ...
    if ((bytes = picoquic_frames_varint_decode(bytes, bytes_max, &msg->message_type)) != NULL) {
        ...
        case QUICRQ_ACTION_REPAIR:
            bytes = quicrq_repair_msg_decode(bytes, bytes_max, &msg->message_type, &msg->group_id, &msg->object_id, &msg->offset, &msg->is_last_fragment, &msg->length, &msg->data);
            break;
...
uint8_t* quicrq_msg_encode(uint8_t* bytes, uint8_t* bytes_max, quicrq_message_t* msg)
{
    ...
    case QUICRQ_ACTION_REQUEST_REPAIR:
        ...
        break;
    case QUICRQ_ACTION_REPAIR:
        bytes = quicrq_repair_msg_encode(bytes, bytes_max, msg->message_type, msg->group_id, msg->object_id, msg->offset, msg->is_last_fragment, msg->length, msg->data);
        break;
    case QUICRQ_ACTION_POST:
